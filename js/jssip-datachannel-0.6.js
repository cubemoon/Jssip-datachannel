...
var JsSIP = (function() {
  ...
  Object.defineProperties(JsSIP, {
    version: {
      get: function(){ return '0.3.1-devel'; }
    },
    name: {
...
RTCMediaHandler.prototype = {
  ...
  */
  init: function (RTCConstraints) {
   ...
   this.peerConnection = new webkitRTCPeerConnection({'iceServers': servers}, {optional: [{RtpDataChannels: true}]});
      
   this.peerConnection.ondatachannel = function (event) {
       console.log('>>>>>>>>>> | Received Channel');
       var receiverChannel = event.channel;

       receiverChannel.onopen = function (event) {
           console.log('>>>>>>>>>> | Channel state is: ' + receiverChannel.readyState);
       };

       receiverChannel.onclose = function (e) {
           console.log('>>>>>>>>>> | Channel state is: ' + receiverChannel.readyState);
       };

       receiverChannel.onmessage = function (eventm) {
           console.log('>>>>>>>>>> | Channel message received: ' + eventm.data);
       };

       self.session.ua.dataChannel = receiverChannel;
   };
  ...
  createDataChannel: function(mediaConstraints) {
	    ...
	    var senderChannel = self.peerConnection.createDataChannel("RTCDataChannel", mediaConstraints);
	    
	    senderChannel.onopen = function(e){
	  	  console.log('>>>>>>>>>> | Channel state is: ' + senderChannel.readyState);
	    };
	    
	    senderChannel.onclose = function (e) {
	  	  console.log('>>>>>>>>>> | Channel state is: ' + senderChannel.readyState);
	    };
	    
	    senderChannel.onmessage = function(eventm){
	    	console.log('>>>>>>>>>> | Channel message received: ' + eventm.data);
	    };
...
RTCSession.prototype.answer = function (options) {
      ...
      var
        ...
        replySucceeded = function () {
            ...
            for (event in eventHandlers) {
                self.on(event, eventHandlers[event]);
            }
  ...
  self.rtcMediaHandler.createAnswer(answerCreationSucceeded, answerCreationFailed);
};
...
RTCSession.prototype.connect = function(target, options) {
  ...
  var event, requestParams,
    ...
    eventHandlers = options.eventHandlers || {},
    extraHeaders = options.extraHeaders || [],
    mediaConstraints = options.mediaConstraints || {reliable: false},
    RTCConstraints = options.RTCConstraints || {optional: [{RtpDataChannels: true}]};
...
RTCSession.prototype.sendInitialRequest = function(mediaConstraints) {
 ...
 offerCreationFailed = function() {
  ...
  self.ua.dataChannel =  self.rtcMediaHandler.createDataChannel(mediaConstraints);
  self.rtcMediaHandler.createOffer(offerCreationSucceeded, offerCreationFailed);
 ...
 *
 */
UA.prototype.connectDataChannel = function(target, options) {
  var session;
...
/**
    ...
    *
    */
UA.prototype.closeDataChannel = function () {
